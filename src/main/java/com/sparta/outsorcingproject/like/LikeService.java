...
public class LikeService {
    ...
    public Page<StoreResponseDto> likesStoreList(User user, int page) {
        ...
        //유저가 좋아요한 상점이 있는지 확인
        Page<Like> likes = likeRepository.findByUserAndContentType(user, LikeContentType.STORE,pageRequest,pageable);
        if (likes.isEmpty()) {
...
//        //  List<StoreResponseDto> -> Page 객체로 변환해서 반환
    ...
    public Page<ReviewResponseDto> likereviewList(User user, int page) {
        ...
        Pageable pageable = pageRequest;
        //유저가 좋아요한 리뷰가 있는지
        Page<Like> likes = likeRepository.findByUserAndContentType(user, LikeContentType.REVIEW, pageRequest,pageable);
        if (likes.isEmpty()) {
